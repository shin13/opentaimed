...
the public APIs of 衛福部食藥署 (Taiwan FDA / TFDA), made for AI assistants.
...
AI assistants often answer drug questions by guessing from training data — and
...
|---|---|---|
| [`taiwan-fda-mcp/`](./taiwan-fda-mcp) | **shipped — v0.2.1** | An MCP server with 3 tools. Works with any MCP client (Claude Desktop, Claude Code, Codex…). |
...
Once it's added to your AI client, just ask a drug question in Chinese. The
easiest install:
...
| 部分 | 狀態 | 是什麼 |
|---|---|---|
| [`taiwan-fda-mcp/`](./taiwan-fda-mcp) | **已上線 — v0.2.1** | 有 3 個工具的 MCP server,可搭配任何 MCP client(Claude Desktop、Claude Code、Codex…)。 |
...
把它加進你的 AI 工具後,直接用中文問藥物問題。最簡單的安裝:
