...
from taiwan_fda_mcp.models import DrugAppearance, DrugLicense
from taiwan_fda_mcp.sources.opendata.dataset37 import parse_rows
from taiwan_fda_mcp.sources.opendata.dataset42 import parse_rows as _parse_rows_42

_logger = logging.getLogger(__name__)

_DATASET37_PATH = "/data/opendata/export/37/json"
_DATASET42_PATH = "/data/opendata/export/42/json"
...
async def fetch_dataset42(
    base_url: str,
    timeout: float = 60.0,  # noqa: ASYNC109
    rate_limit_interval: float = 0.0,
) -> list[DrugAppearance]:
    """Download, unzip, and parse the Dataset 42 (藥品外觀) export.

    Args:
        base_url: e.g. 'https://data.fda.gov.tw'
        timeout: per-request timeout in seconds.
        rate_limit_interval: seconds to sleep AFTER the request (good-citizen
            throttle). 0 disables it (the default, for tests).

    Returns:
        List of DrugAppearance.

    Raises:
        DatasetFetchError: HTTP failure, malformed zip, or parse failure.
    """
    url = f"{base_url.rstrip('/')}{_DATASET42_PATH}"
    _logger.info("dataset42.fetch.start", extra={"url": url})
    ...
    try:
        ...
        raise DatasetFetchError(
            RCode.DATASET_FETCH_FAILED,
            f"Failed to download Dataset 42: {exc}",
        ) from exc
    finally:
        if rate_limit_interval > 0:
            await asyncio.sleep(rate_limit_interval)
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
            json_member = next((n for n in zf.namelist() if n.endswith(".json")), None)
            if json_member is None:
                raise DatasetFetchError(
                    RCode.DATASET_PARSE_FAILED,
                    "Dataset 42 ZIP contains no .json member",
                )
            raw_json = zf.read(json_member)
    except zipfile.BadZipFile as exc:
        raise DatasetFetchError(
            RCode.DATASET_PARSE_FAILED,
            "Dataset 42 download is not a valid ZIP",
        ) from exc
    ...
    try:
        ...
        raise DatasetFetchError(
            RCode.DATASET_PARSE_FAILED,
            "Dataset 42 JSON could not be decoded",
        ) from exc

    rows = _parse_rows_42(raw_rows)
    _logger.info("dataset42.fetch.done", extra={"count": len(rows)})
    return rows
